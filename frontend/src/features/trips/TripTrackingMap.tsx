...
export function TripTrackingMap({ trip }: TripTrackingMapProps) {
    const mapContainerRef = useRef<HTMLDivElement>(null);
    const mapRef = useRef<L.Map | null>(null);

    useEffect(() => {
        if (!mapContainerRef.current || !trip) return;
        ...
        const originCoords: [number, number] = [trip.originCoords.lat, trip.originCoords.lng];
        const destCoords: [number, number] = [trip.destinationCoords.lat, trip.destinationCoords.lng];
        ...
        const progressRatio = (trip.progress || 0) / 100;
        ...
        const bounds = L.latLngBounds(routeWaypoints).pad(0.2);

        mapRef.current = L.map(mapContainerRef.current, { zoomControl: false, scrollWheelZoom: true, dragging: true }).fitBounds(bounds);

        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(mapRef.current);
        L.control.zoom({ position: 'topleft' }).addTo(mapRef.current);

        const originIcon = L.divIcon({ html: `<svg width="24" height="24" viewBox="0 0 24 24"><circle cx="12" cy="12" r="8" fill="#10b981" stroke="white" stroke-width="3"/><circle cx="12" cy="12" r="3" fill="white"/></svg>`, iconSize: [24, 24], className: 'map-icon' });
        const destinationIcon = L.divIcon({ html: `<svg width="28" height="36" viewBox="0 0 24 32"><path d="M12 0C7.58 0 4 3.58 4 8c0 5.25 8 16 8 16s8-10.75 8-16c0-4.42-3.58-8-8-8z" fill="#ef4444" stroke="white" stroke-width="2"/><circle cx="12" cy="8" r="3" fill="white"/></svg>`, iconSize: [28, 36], iconAnchor: [14, 36], className: 'map-icon' });
        ...
        L.marker(originCoords, { icon: originIcon }).bindPopup(`<b>مبدأ:</b> ${trip.origin}`).addTo(mapRef.current);
        L.marker(destCoords, { icon: destinationIcon }).bindPopup(`<b>مقصد:</b> ${trip.destination}`).addTo(mapRef.current);

        if (progressRatio > 0) {
            const completedWaypoints = routeWaypoints.slice(0, currentSegmentIndex + 1);
            if (segmentProgress > 0 && currentSegmentIndex < routeWaypoints.length - 1) completedWaypoints.push(currentPosition);
        ...
        if (progressRatio > 0 && progressRatio < 1) {
            L.marker(currentPosition, { icon: truckIcon }).bindPopup(`<b>موقعیت فعلی</b><br/>${trip.progress}% مسیر طی شده`).addTo(mapRef.current);
        }
    ...
    return <div ref={mapContainerRef} className="w-full h-full bg-muted" />;
}